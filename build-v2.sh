...
alias BUF_RUN='docker run --rm -u $(id -u):$(id -g) -v $(pwd):/workdir docker.ninjinlab.xyz/buf-gen:latest'
...
function bufGenerate {
  ...
  case $lang in 
    go)
      BUF_RUN generate --template buf.gen.go.yaml -o $target/out $target
    ;;
  esac
  leaveDir
}
